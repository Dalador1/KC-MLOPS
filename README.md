...
Web-интерфейс доступен по адресу `http://localhost/`. В нём реализованы:

- главная страница с описанием сервиса;
- регистрация и вход по JWT в `HttpOnly` cookie;
- просмотр и пополнение баланса;
- отправка нескольких писем на асинхронную проверку;
- отдельный вывод предсказаний и ошибок валидации;
- история ML-запросов, их результатов и транзакций.

Web-интерфейс построен на Jinja2 и обычных HTML-формах без JavaScript. Формы обращаются к тому же сервисному слою, что и REST API, поэтому бизнес-логика не дублируется. Асинхронная страница задачи обновляется стандартным HTML `meta refresh`. CORS настраивается переменной `CORS_ORIGINS`. HTTP middleware добавляет в ответы заголовки `X-Request-ID` и `X-Process-Time` и журналирует запросы.
